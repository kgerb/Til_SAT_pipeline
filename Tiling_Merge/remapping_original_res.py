...
def main(original_file, subsampled_file, output_file):
    ...
    subsampled_points = np.vstack((subsampled_las.x, subsampled_las.y, subsampled_las.z)).T
    ...
    if "PredInstance" not in existing_dims:
        original_las.add_extra_dim(laspy.ExtraBytesParams(name="PredInstance", type=np.int32))
    if "PredSemantic" not in existing_dims:
        original_las.add_extra_dim(laspy.ExtraBytesParams(name="PredSemantic", type=np.int32))
    ...
    print(f"Original point cloud now has the following attributes:")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Map attributes from a subsampled point cloud back to the original point cloud.')
    parser.add_argument('--original_file', type=str, help='Path to the original point cloud file')
    parser.add_argument('--subsampled_file', type=str, help='Path to the subsampled point cloud file with attributes')
    parser.add_argument('--output_file', type=str, help='Path to save the updated original point cloud file')
