...
SCRIPT_DIR="/workspace/SegmentAnyTree"
...
python3.8 "$SCRIPT_DIR/nibio_inference/fix_naming_of_input_files.py" "$DEST_DIR/input_data"

# UTM normalization 
python3.8 "$SCRIPT_DIR/nibio_inference/pipeline_utm2local_parallel.py" -i "$DEST_DIR/input_data" -o "$DEST_DIR/utm2local"

# Update the eval.yaml file with the correct paths
cp "$SCRIPT_DIR/conf/eval.yaml" "$DEST_DIR"
python3.8 "$SCRIPT_DIR/nibio_inference/modify_eval.py" "$DEST_DIR/eval.yaml" "$DEST_DIR/utm2local" "$DEST_DIR"

# clear cache
python3.8 "$SCRIPT_DIR/nibio_inference/clear_cache.py" --eval_yaml "$DEST_DIR/eval.yaml"

# Run the inference script with the config file
python3.8 eval.py --config-name "$DEST_DIR/eval.yaml"
...
python3.8 "$SCRIPT_DIR/nibio_inference/rename_result_files_instance.py" "$DEST_DIR/eval.yaml" "$DEST_DIR"

# Rename segmentation files
python3.8 "$SCRIPT_DIR/nibio_inference/rename_result_files_segmentation.py" "$DEST_DIR/eval.yaml" "$DEST_DIR"
