...
class InstanceSegmentationMetrics:
    ...
    def __init__(
        ...
        self.skip_flag = self.check_if_labels_exist(
            X_label=self.GT_LABEL_NAME, Y_label=self.TARGET_LABEL_NAME
        )
        ...
        if not self.skip_flag:
            ...
            self.Y_labels = self.instance_segmented_las[self.TARGET_LABEL_NAME].astype(
                int
            )
    ...
    def check_if_labels_exist(self, X_label="treeID", Y_label="instance_nr"):
        ...
        if (
            Y_label
            not in self.instance_segmented_las.header.point_format.dimension_names
        ):
            skip_flag = True
    ...
    def do_knn_mapping(self):
        ...
        residual_ind = np.delete(
            np.arange(X.shape[0]), ind.reshape(-1)
    ...
    def find_dominant_classes_in_gt(self, input_file):
        ...
        tmp_dict_sorted = {
            k: v
            for k, v in sorted(tmp_dict.items(), key=lambda item: item[1], reverse=True)
        }
    ...
    def remove_dominant_label(
        self, dominant_labels_sorted, dominant_label_key, dominant_label
    ):
    ...
    def iterate_over_pc(self):
        ...
        for gt_class in gt_classes_to_iterate:
            ...
            self.remove_dominant_label(
                dominant_labels_sorted, dominant_label_key, dominant_label
            )
            label_mapping_dict[dominant_label_key] = dominant_label
    ...
    def compute_metrics(self):
        ...
        if not self.skip_flag:
            ...
            for label in list(label_mapping_dict.keys()):
                ...
                false_negative = np.where(
                    self.X_labels[self.dict_Y["residual_ind"]] == dominant_label
                )[0].shape[0]

                ## true negative
                true_negative = (
                    self.dict_Y["X"].shape[0]
                    - false_negative
                    - true_positive
                    - false_positive
                )
                ...
                Z_values = np.array(self.instance_segmented_las.z)[
                    self.Y_labels == label
                ]
        ...
        if metric_dict:
            for label in metric_dict.keys():
                for parameter in interesting_parameters:
                    metric_dict_weighted_by_tree_hight[parameter] += (
                        metric_dict[label]["high_of_tree_gt"]
                        * metric_dict[label][parameter]
                    )
            # divide by the sum of the hights of the trees
            for parameter in interesting_parameters:
                metric_dict_weighted_by_tree_hight[parameter] /= sum(
                    [
                        metric_dict[label]["high_of_tree_gt"]
                        for label in metric_dict.keys()
                    ]
                ...
                if parameter == "rmse_hight":
                    ...
                    metric_dict_weighted_by_tree_hight[parameter] = (
                        metric_dict_weighted_by_tree_hight[parameter] ** 0.5
                    )
        ...
        if metric_dict:
            ...
            for parameter in interesting_parameters:
                ...
                metric_dict_mean[parameter] = metric_dict_mean[parameter] / len(
                    metric_dict
                )  # local metric
        ...
        if metric_dict:
            ...
            trees_predicted = np.unique(
                [
                    metric_dict[key]["gt_label(dominant_label)"]
            ...
            trees_correctly_predicted_IoU = np.unique(
                [
                    metric_dict[key]["gt_label(dominant_label)"]
                    for key in metric_dict.keys()
                    if metric_dict[key]["IoU"] > 0.5
                ]
            )
            ...
            tree_level_metric = {
                ...
                / len(gt_trees),
                "false_positve (commission)": len(
                    trees_predicted - trees_correctly_predicted_IoU
                )
                / len(trees_predicted),
                "false_negative (omissions)": len(
                    gt_trees - trees_correctly_predicted_IoU
                )
                / len(gt_trees),
                "gt": len(gt_trees),
            }
            ...
            if self.verbose:
                ...
                print(
                    f"Trees correctly predicted with IoU > 0.5: {trees_correctly_predicted_IoU}"
                )
    ...
    def main(self):
        metric_dict, metric_dict_weighted_by_tree_hight, metric_dict_mean = (
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--remove_ground",
        action="store_true",
        help="Do not take into account the ground (class 0).",
        default=False,
    )
    parser.add_argument(
        "--csv_file_name",
        type=str,
        help="Name of the csv file to save the metrics to",
        default=None,
    )
    parser.add_argument(
        "--verbose",
