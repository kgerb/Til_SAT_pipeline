...
class MetricSemSeg:
    ...
    def get_file_name_list(self):
        ...
        for file_name in file_name_list_original:
            file_name_list_original_core.append(
        ...
        for file in os.listdir(self.pred_folder):
            ...
            if (
                os.path.basename(file).split(".")[0] in file_name_list_original_core
                and os.path.basename(file).split(".")[1] == "segmented"
            ):
                print("processing file {}".format(file))
                # check is the file is a las file
                if file.endswith(".las"):
                    file_name_list_predicted.append(
                        os.path.join(self.pred_folder, file)
                    )
                # check if the file is a ply file and there is no las file with the same core name
                elif file.endswith(".ply") and (
                    os.path.basename(file).split(".")[0] + ".segmented.las"
                    not in os.listdir(self.pred_folder)
                ):
                    ...
                    os.system(
                        "pdal translate {} {} --writers.las.dataformat_id=3 --writers.las.extra_dims=all".format(
                            file_name, file_name_las
                        )
                    )
    ...
    def get_metrics_for_single_file(self, file_name_original, file_name_predicted):
        ...
        # get labels
        labels_predicted = self.get_labels_from_point_file(
            file_name_predicted, predicted=True
        )
        ...
        if self.verbose:
            print(
                "labels_predicted range: ",
                np.min(labels_predicted),
                np.max(labels_predicted),
            )
            print(
                "labels_original range: ",
                np.min(labels_original),
                np.max(labels_original),
            )
        ...
        # get points
        xyz_predicted = self.get_xyz_from_point_file(
            file_name_predicted, predicted=True
        )
        ...
        conf_matrix = np.round(
            confusion_matrix(
                labels_original_closest, labels_predicted, normalize="true"
            ),
            decimals=2,
        )
        ...
        precision = precision_score(
            labels_original_closest, labels_predicted, average="weighted"
        )
        ...
        for name in class_names:
            precision_per_class[name] = precision_score(
                labels_original_closest,
                labels_predicted,
                labels=[class_names.index(name)],
                average="weighted",
            )
            precision_per_class[name] = np.round(precision_per_class[name], decimals=3)
            recall_per_class[name] = recall_score(
                labels_original_closest,
                labels_predicted,
                labels=[class_names.index(name)],
                average="weighted",
    ...
    def get_metrics_for_all_files(self):
        ...
        results = Parallel(n_jobs=-1, verbose=0)(
            delayed(self.get_metrics_for_single_file)(
                file_name_original, file_name_predicted
            )
            for file_name_original, file_name_predicted in file_name_list
        )
        ...
        if self.plot_confusion_matrix:
            disp = ConfusionMatrixDisplay(
                confusion_matrix=conf_matrix_mean, display_labels=class_names
            )
            disp.plot()
        ...
        # loop over all items
        for precision_per_class, recall_per_class, f1_per_class in zip(
            precision_per_class_list, recall_per_class_list, f1_per_class_list
        ):
        ...
        for name in class_names:
            precision_per_class_list_mean_3[name] = np.mean(
                precision_per_class_list_3[name]
            )
        ...
        for name in class_names:
            precision_per_class_list_mean_4[name] = np.mean(
                precision_per_class_list_4[name]
            )
        ...
        for name in class_names:
            ...
            if (
                precision_per_class_list_mean_3[name] != 0
                and precision_per_class_list_mean_4[name] != 0
            ):
                precision_per_class_mean[name] = (
                    precision_per_class_list_mean_3[name]
                    + precision_per_class_list_mean_4[name]
                ) / 2
            ...
            if (
                recall_per_class_list_mean_3[name] != 0
                and recall_per_class_list_mean_4[name] != 0
            ):
                recall_per_class_mean[name] = (
                    recall_per_class_list_mean_3[name]
                    + recall_per_class_list_mean_4[name]
                ) / 2
            # if one of the items is 0, take the other one
            ...
            if (
                f1_per_class_list_mean_3[name] != 0
                and f1_per_class_list_mean_4[name] != 0
            ):
                f1_per_class_mean[name] = (
                    f1_per_class_list_mean_3[name] + f1_per_class_list_mean_4[name]
                ) / 2
            # if one of the items is 0, take the other one
        ...
        if self.verbose:
            ...
            print(
                ...
                os.path.join(self.gt_folder, "results.json"),
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="path to the original point clouds directory",
    )
    parser.add_argument(
        "--path_predicted",
        type=str,
        default="data/predicted",
        help="path to the predicted point clouds directory",
    )
    parser.add_argument(
        "--adjust_gt_data",
        action="store_true",
        default=False,
        help="adjust the ground truth data",
    )
    parser.add_argument(
