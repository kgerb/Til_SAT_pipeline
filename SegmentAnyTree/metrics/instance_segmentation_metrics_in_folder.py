...
class InstanceSegmentationMetricsInFolder:
    ...
    def main(self):
        ...
        gt_ply_file_paths = glob.glob(
            self.gt_las_folder_path + "/*.ply", recursive=False
        )
        ...
        for gt_ply_file_path in gt_ply_file_paths:
            ...
            jaklas.write(
                points_df,
                gt_ply_file_path.replace(".ply", ".las"),
                point_format=2,
                scale=(0.001, 0.001, 0.001),
            )

        # get all las files in the gt_las_folder_path
        gt_las_file_paths = glob.glob(
            self.gt_las_folder_path + "/*.las", recursive=False
        )
        # account also for laz files
        gt_las_file_paths += glob.glob(
            self.gt_las_folder_path + "/*.laz", recursive=False
        )

        gt_las_file_paths.sort()

        # get all ply files in the target_las_folder_path
        target_ply_file_paths = glob.glob(
            self.target_las_folder_path + "/*.ply", recursive=False
        )
        ...
        for target_ply_file_path in target_ply_file_paths:
            ...
            jaklas.write(
                points_df,
                target_ply_file_path.replace(".ply", ".las"),
                point_format=2,
                scale=(0.001, 0.001, 0.001),
            )

        # get all las files in the target_las_folder_path
        target_las_file_paths = glob.glob(
            self.target_las_folder_path + "/*.las", recursive=False
        )
        # account also for laz files
        target_las_file_paths += glob.glob(
            self.target_las_folder_path + "/*.laz", recursive=False
        )
        ...
        if len(gt_las_file_paths) != len(target_las_file_paths):
            ...
            print(
                "Number of files in gt_las_folder_path: " + str(len(gt_las_file_paths))
            )
            print(
                "Number of files in target_las_folder_path: "
                + str(len(target_las_file_paths))
            )
            raise Exception(
                "The number of las files in the gt_las_folder_path and target_las_folder_path are not the same"
            )

        # iterate over the las files
        for gt_las_file_path, target_las_file_path in zip(
            gt_las_file_paths, target_las_file_paths
        ):
            ...
            if len(target_las_file.points) == 0:
                ...
                if self.verbose:
                    print(
                        "Removed empty las file from the list: " + target_las_file_path
                    )
        ...
        for gt_las_file_path, target_las_file_path in zip(
            ...
            target_las_file_core_name = os.path.basename(target_las_file_path).split(
                "."
            )[0]
        ...
        if len(matched_paths) != len(gt_las_file_paths) or len(matched_paths) != len(
            target_las_file_paths
        ):
            # print names of the folders
            print("gt_las_folder_path: " + self.gt_las_folder_path)
            print("target_las_folder_path: " + self.target_las_folder_path)
        ...
        parallel_output = Parallel(n_jobs=-1, verbose=0)(
            delayed(self.compute_metrics)(gt_las_file_path, target_las_file_path)
        ...
        if self.output_folder_path is not None:
            ...
            save_to_csv_path = os.path.join(
                self.output_folder_path, "summary_metrics_all_plots.csv"
            )
    ...
    def compute_metrics(self, gt_las_file_path, target_las_file_path):
        ...
        if self.output_folder_path is not None:
            # create the output folder path
            save_to_csv_path = os.path.join(
                self.output_folder_path, gt_las_file_core_name + ".csv"
            )
        ...
        metric_dict, metric_dict_weighted_by_tree_hight, metric_dict_mean = (
            instance_segmentation_metrics.main()
        )
