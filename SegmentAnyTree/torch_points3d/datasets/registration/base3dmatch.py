...
class Base3DMatch(Dataset):
    ...
    def __init__(
        ...
        super(Base3DMatch, self).__init__(root, transform, pre_transform, pre_filter)
    ...
    def _create_fragment(self, mod):
        ...
        if files_exist(
            [osp.join(self.processed_dir, mod, "raw_fragment")]
        ):  # pragma: no cover
        ...
        for scene_path in os.listdir(osp.join(self.raw_dir, mod)):
            ...
            list_seq = [
                f
                for f in os.listdir(osp.join(self.raw_dir, mod, scene_path))
                if "seq" in f
            ]
            for seq in list_seq:
                frames_dir = osp.join(self.raw_dir, self.mode, scene_path, seq)
                out_dir = osp.join(
                    self.processed_dir, mod, "raw_fragment", scene_path, seq
                )
                makedirs(out_dir)
                path_intrinsic = osp.join(
                    self.raw_dir, self.mode, scene_path, "camera-intrinsics.txt"
                )
                ...
                if not self.is_fine:
                    ...
                    assert len(list_path_frames) == len(list_path_trans), log.error(
                        "For the sequence {},"
                        "the number of frame "
                        "and the number of "
    ...
    def _pre_transform_fragment(self, mod):
        ...
        for scene_path in os.listdir(osp.join(self.raw_dir, mod)):
            ...
            for seq in list_seq:
                ...
                out_dir = osp.join(self.processed_dir, mod, "fragment", scene_path, seq)
                makedirs(out_dir)
    ...
    def _compute_matches_between_fragments(self, mod):
        ...
        for scene_path in os.listdir(osp.join(self.raw_dir, mod)):
            ...
            for seq in list_seq:
                ...
                fragment_dir = osp.join(
                    self.processed_dir, mod, "fragment", scene_path, seq
                )
                list_fragment_path = sorted(
                    [
                        osp.join(fragment_dir, f)
                ...
                for path1 in list_fragment_path:
                    for path2 in list_fragment_path:
                        if path1 < path2:
                            ...
                            if (
                                np.max(match["overlap"]) > self.min_overlap_ratio
                                and np.max(match["overlap"]) < self.max_overlap_ratio
                            ):
    ...
    def _save_patches(self, mod):
        ...
        for i in range(len(os.listdir(match_dir))):
            match = np.load(
                osp.join(match_dir, "matches{:06d}.npy".format(i)), allow_pickle=True
            ).item()
            ...
            for _ in range(self.num_random_pt):
                ...
                if self.pre_filter is not None:
                    if self.pre_filter(data_source) and self.pre_filter(data_target):
                        torch.save(
                            data_source,
                            osp.join(out_dir, "patches_source{:06d}.pt".format(idx)),
                        )
                        torch.save(
                            data_target,
                            osp.join(out_dir, "patches_target{:06d}.pt".format(idx)),
                        )
                        idx += 1
                else:
                    torch.save(
