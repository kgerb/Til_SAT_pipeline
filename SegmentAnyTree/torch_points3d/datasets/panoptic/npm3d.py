...
from torch_points3d.datasets.segmentation.npm3d import (
    NPM3DSphere,
    NPM3DCylinder,
    INV_OBJECT_LABEL,
)
...
def final_eval(pre_sem, pre_ins_embed, pre_ins_offset, pos, gt_sem, gt_ins):
        ...
        for ip, ins_pred in enumerate(pts_in_pred[i_sem]):
            ...
            if not pts_in_gt[i_sem]:
                ...
                val_name = os.path.join(
                    "viz_for_fp_pre", "sem" + str(i_sem) + "_fp" + str(ip)
                )
                write_ply(
                    val_name,
            ...
            if ovmax >= at:
                ...
                val_name = os.path.join(
                    "viz_for_tp_pre", "sem" + str(i_sem) + "_tp" + str(ip)
                )
                write_ply(
                    val_name,
                    [pos_ins[ins_pred], pred_sem[ins_pred].astype("int32")],
    ...
    log_string(
        "Instance Segmentation meanRQ: {}".format(np.mean(RQ[sem_classcount_final]))
    )
    log_string("Instance Segmentation SQ: {}".format(SQ[sem_classcount]))
    log_string(
        "Instance Segmentation meanSQ: {}".format(np.mean(SQ[sem_classcount_final]))
    )
    log_string("Instance Segmentation PQ: {}".format(PQ[sem_classcount]))
    log_string(
        "Instance Segmentation meanPQ: {}".format(np.mean(PQ[sem_classcount_final]))
    )
    log_string("Instance Segmentation PQ star: {}".format(PQStar[sem_classcount]))
    log_string(
        "Instance Segmentation mean PQ star: {}".format(
            np.mean(PQStar[sem_classcount_final])
        )
    )
...
class NPM3DFusedDataset(BaseDataset):
    ...
    def __init__(self, dataset_opt):
        ...
        dataset_cls = (
            PanopticNPM3DCylinder
            if sampling_format == "cylinder"
            else PanopticNPM3DSphere
        )
