...
class NPM3DOriginalFused(InMemoryDataset):
    ...
    def __init__(
        ...
        super(NPM3DOriginalFused, self).__init__(
    ...
    def processed_dir(self):
        ...
        if isinstance(self.test_area, int):
            return osp.join(
                self.root,
                "processed_" + str(self.grid_size) + "_" + str(self.test_area),
            )
        else:
            return osp.join(
                self.root,
                "processed_" + str(self.grid_size) + "_" + str(self.area_name),
            )
    ...
    def raw_areas_paths(self):
        ...
        if isinstance(self.test_area, int):
            return [
                os.path.join(self.processed_dir, "raw_area_%i.pt" % i) for i in range(8)
            ]
        else:
            return [
                os.path.join(self.processed_dir, "raw_area_" + self.area_name + ".pt")
            ]
    ...
    def process(self):
        if not os.path.exists(self.pre_processed_path):
            input_ply_files = [
                osp.join(self.raw_dir, f + ".ply") for f in ALL_FILE_NAMES
            ]
            ...
            for area_num, file_path in enumerate(input_ply_files):
                ...
                if "val" in area_name:  # area_num == self.test_area-1:
                    ...
                    FILE_NAMES[self.test_area - 1] in area_name and "test" in area_name
                ):
                    is_test_data = True
                elif (
                    FILE_NAMES[self.test_area - 1] not in area_name
                    and "train" in area_name
                ):
                    is_train_data = True
