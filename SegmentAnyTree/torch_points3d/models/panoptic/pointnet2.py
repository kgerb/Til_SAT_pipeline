...
class PointNet2_D(UnetBasedModel):
    ...
    def __init__(self, option, model_type, dataset, modules):
        ...
        self.Offset.append(
            Conv1D(offset_mlp_opt.nn[-1], 3, activation=None, bias=True, bn=False)
        )
    ...
    def forward(self, epoch=-1, step=-1, is_training=True, **kwargs):
        ...
        semantic_logits = (
            self.Semantic(last_feature)
            .transpose(1, 2)
            .contiguous()
            .view((-1, self._num_classes))
        )
        offset_logits = (
            self.Offset(last_feature).transpose(1, 2).contiguous().view((-1, 3))
        )
        embedding_logits = (
            self.Embedding(last_feature)
            .transpose(1, 2)
            .contiguous()
            .view((-1, self.opt.mlp_ins.embed_dim))
        )
    ...
    def _dump_visuals(self, epoch):
        if random.random() < self.opt.vizual_ratio:
            ...
            data_visual = Data(
                pos=self.raw_pos,
                y=self.labels.y,
                instance_labels=self.labels.instance_labels,
