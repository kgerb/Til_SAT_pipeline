...
class PointGroup3heads(BaseModel):
    ...
    def __init__(self, option, model_type, dataset, modules):
        ...
        self.LogSoftLay = Seq().append(torch.nn.LogSoftmax(dim=-1))
        self.loss_names = [
            "loss",
            "offset_norm_loss",
            "offset_dir_loss",
            ...
            if epoch > self.opt.prepare_epoch:  # Active by default epoch > -1: #
                if self.opt.cluster_type == 1:
                    ...
                    all_clusters, cluster_type = self._cluster5(
                        semantic_logits, offset_logits, embed_logits
                    )
    ...
    def _cluster7(self, semantic_logits, offset_logits, embed_logits):
        ...
        for i in self._thing_classes:
            predicted_labels[predicted_labels_copy == i] = min_thing_label.to(
                self.device
            )
        clusters_pos = []
        clusters_pos = region_grow(
    ...
    def _compute_loss(self, epoch):
        ...
        if self._weight_per_point != None:
            if self._weight_per_point == "height":
                ...
                sample_weight = above_height / (
                    above_height.mean() + np.finfo(np.float64).eps
                )
        ...
        if self.use_binary_loss:
            ...
            bi_y[
                (bi_y[..., None] == self._stuff_classes[1:].to(self.device)).any(-1)
            ] = 0
            ...
            self.semantic_loss_bi = torch.nn.functional.nll_loss(
                self.output.bi_semantic_logits,
                (bi_y).to(torch.int64),
                ignore_index=IGNORE_LABEL,
            )
