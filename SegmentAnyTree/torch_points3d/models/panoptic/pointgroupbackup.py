...
class PointGroupEmbed(BaseModel):
    ...
    def meanshift_torch(self, prediction, bandwidth):
        ...
        def gaussian(dist, bandwidth):
            return torch.exp(-0.5 * (dist / bandwidth) ** 2) / (
                bandwidth * torch.sqrt(torch.tensor(2 * math.pi).to(self.device))
            )
    ...
    def meanshift_torch2(self, prediction, bandwidth, bs=500, iteration=500):
        ...
        def aligned_op(x, y, f):
            return f(*align(x, y, 0))

        def div(x, y):
            return aligned_op(x, y, operator.truediv)

        def add(x, y):
            return aligned_op(x, y, operator.add)

        def sub(x, y):
            return aligned_op(x, y, operator.sub)
    ...
    def _cluster(self, semantic_logits, embed_logits):
        ...
        for s in batch_size:
            ...
            num_clusters_embed, pre_ins_labels_embed = self.meanshift_cluster(
            ...
            predicted_ins_labels_byEmbed[sampleInBatch_local_ind] = (
                pre_ins_labels_embed + instance_num_embed
            )
    ...
    def _cluster3(self, semantic_logits, embed_logits):
        ...
        predicted_labels = (
            torch.max(semantic_logits, 1)[1].cpu().detach().numpy()
        )  # [N]
        ind = np.arange(0, N)
        unique_predicted_labels = np.unique(predicted_labels)
        ignore_labels = self._stuff_classes.cpu().detach().numpy()
        label_mask = (
            torch.ones(predicted_labels.size, dtype=torch.bool).cpu().detach().numpy()
        )
        for l in unique_predicted_labels:
            if l in ignore_labels:
                # Build clusters for a given label (ignore other points)
        ...
        embed_logits_logits_u = (
            torch.cat((self.raw_pos[label_mask], embed_logits[label_mask]), 1)
            .cpu()
            .detach()
            .numpy()
        )
    ...
    def _dump_visuals(self, epoch):
        if random.random() < self.opt.vizual_ratio:
            ...
            if not os.path.exists("viz"):
                os.mkdir("viz")
            torch.save(
                data_visual.to("cpu"), "viz/data_e%i_%i.pt" % (epoch, self.visual_count)
            )
            self.visual_count += 1
