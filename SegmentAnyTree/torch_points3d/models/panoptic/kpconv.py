...
class KPConvPaper(UnwrappedUnetBasedModel):
                ...
                self.Semantic.add_module(
                    ...
                    Sequential(
                        *[
                            ...
                            FastBatchNorm1d(
                                cls_mlp_opt.nn[i], momentum=cls_mlp_opt.bn_momentum
                            ),
                            LeakyReLU(0.2),
                        ]
                    ),
                ...
                self.Offset.add_module(
                    ...
                    Sequential(
                        *[
                            ...
                            FastBatchNorm1d(
                                offset_mlp_opt.nn[i],
                                momentum=offset_mlp_opt.bn_momentum,
                            ),
                            LeakyReLU(0.2),
                        ]
                    ),
                ...
                self.Embedding.add_module(
                    ...
                    Sequential(
                        *[
                            ...
                            FastBatchNorm1d(
                                ins_mlp_opt.nn[i], momentum=ins_mlp_opt.bn_momentum
                            ),
                            LeakyReLU(0.2),
                        ]
                    ),
    ...
    def set_input(self, data, device):
        ...
        all_labels = {
            l: data[l] for l in self.__REQUIRED_LABELS__
        }  # .to(device) for l in self.__REQUIRED_LABELS__}
    ...
    def forward(self, epoch=-1, step=-1, is_training=True, **kwargs) -> Any:
        ...
        with torch.no_grad():
            if is_training:
                ...
                embed_clusters, offset_clusters, embed_pre, offset_pre = (
                    self._cluster_3(semantic_logits, embedding_logits, offset_logits)
                )
    ...
    def _cluster(self, semantic_logits, embedding_logits, offset_logits):
        ...
        embed_clusters = []
        offset_clusters = []
        predicted_ins_labels_byEmbed = -1 * torch.ones(
            predicted_labels.size(), dtype=torch.int64
        )
        predicted_ins_labels_byOffset = -1 * torch.ones(
            predicted_labels.size(), dtype=torch.int64
        )
        ...
        for l in unique_predicted_labels:
            ...
            for s in batch_size:  # range(batch_size):
                ...
                predicted_ins_labels_byOffset[sampleInBatch_local_ind] = (
                    t_pre_ins_labels + instance_num_offset
                )
                ...
                predicted_ins_labels_byEmbed[sampleInBatch_local_ind] = (
                    t_pre_ins_labels2 + instance_num_embed
                )
        ...
        return (
            embed_clusters,
            offset_clusters,
            predicted_ins_labels_byEmbed,
            predicted_ins_labels_byOffset,
        )
    ...
    def _cluster_2(self, semantic_logits, embedding_logits, offset_logits):
        ...
        instance_num_embed = 0
        instance_num_offset = 0
        label_mask = torch.ones(predicted_labels.size(), dtype=torch.bool).to(
            self.device
        )
        for l in unique_predicted_labels:
            if l in ignore_labels:
    ...
    def point_grouping(
        self, pos, labels, batch, ignore_labels=[], nsample=300, radius=0.03
    ):
        ...
        if torch.sum(self.input.instance_mask) > 1:
            ...
            for loss_name, loss in discriminative_losses.items():
                ...
                if loss_name == "ins_loss":
                    self.loss += (
                        self.opt.loss_weights.embedding_loss * loss
                    )  # discriminative_losses.items()[0]
    ...
    def _dump_visuals_fortest(self, epoch):
        if 0 < self.opt.vizual_ratio:  # random.random() < self.opt.vizual_ratio:
            ...
            data_visual_fore.embedding = self.output.embedding_logits[
                self.input.instance_mask
            ]
            ...
            for s in batch_size:
                ...
                write_ply(
                    ...
                    [
                        ...
                        data_visual.y[batch_mask_com]
                        .detach()
                        .cpu()
                        .numpy()
                        .astype("int32"),
                        data_visual.instance_labels[batch_mask_com]
                        .detach()
                        .cpu()
                        .numpy()
                        .astype("int32"),
                        data_visual.semantic_prob[batch_mask_com]
                        .detach()
                        .cpu()
                        .numpy(),
                        data_visual.embedding[batch_mask_com].detach().cpu().numpy(),
                        data_visual.vote[batch_mask_com]
                        .detach()
                        .cpu()
                        .numpy()
                        .astype("int32"),
                        data_visual.semantic_pred[batch_mask_com]
                        .detach()
                        .cpu()
                        .numpy()
                        .astype("int32"),
                ...
                write_ply(
                    ...
                    [
                        ...
                        data_visual_fore.y[batch_mask]
                        .detach()
                        .cpu()
                        .numpy()
                        .astype("int32"),
                        data_visual_fore.instance_labels[batch_mask]
                        .detach()
                        .cpu()
                        .numpy()
                        .astype("int32"),
                        ...
                        data_visual_fore.pos[batch_mask].detach().cpu().numpy()
                        + data_visual_fore.vote_label[batch_mask]
                        .detach()
                        .cpu()
                        .numpy(),
                        data_visual_fore.pre_ins[batch_mask]
                        .detach()
                        .cpu()
                        .numpy()
                        .astype("int32"),
                        data_visual_fore.pre_ins2[batch_mask]
                        .detach()
                        .cpu()
                        .numpy()
                        .astype("int32"),
                ...
                write_ply(
                    ...
                    [
                        ...
                        data_visual_fore.vote_label[batch_mask, 0]
                        .detach()
                        .cpu()
                        .numpy(),
                        data_visual_fore.vote[batch_mask, 1].detach().cpu().numpy(),
                        data_visual_fore.vote_label[batch_mask, 1]
                        .detach()
                        .cpu()
                        .numpy(),
                        data_visual_fore.vote[batch_mask, 2].detach().cpu().numpy(),
                        data_visual_fore.vote_label[batch_mask, 2]
                        .detach()
                        .cpu()
                        .numpy(),
                    ],
                ...
                clustering = MeanShift(bandwidth=self.opt.bandwidth).fit(
                    (
                        data_visual_fore.pos[batch_mask].detach().cpu()
                        + data_visual_fore.vote[batch_mask].detach().cpu()
                    )
                )
                pre_inslab = clustering.labels_
                write_ply(
