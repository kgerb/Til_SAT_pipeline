...
def instance_ious(
    ...
    if not cal_iou_based_on_mask:
        ...
        offset_num_gt_instances = torch.cat(
            [torch.tensor([0]), num_gt_instances.cumsum(0)]
        )
        for proposed_instance in range(num_proposed_instances):
            instance = proposal_idx[
                instance_offsets[proposed_instance] : instance_offsets[
                    proposed_instance + 1
                ]
            ]
            ...
            for instance_id in numba.prange(1, sample_instance_count + 1):
                ...
                intersection = torch.logical_and(
                    mask_score > 0.5, instance_labels[instance] == instance_id
                ).sum()
                ious[proposed_instance, gt_count_offset + instance_id - 1] = (
                    intersection
                    / float(
                        proposal_total
...
def instance_iou_loss(
    ...
    shat[higher_mask] = 1
    shat[middle_mask] = (iou_middle - min_iou_threshold) / (
...
def mask_loss(
    ...
    for i, pred_cluster in enumerate(predicted_clusters):
        ...
        if max_ious[i] > 0.5:
            max_iou_idx = (
                max_iou_idxes[i]
                - offset_num_gt_instances[
                    max_iou_idxes[i] > offset_num_gt_instances.cuda()
                ][-1]
            )
    ...
    return torch.nn.functional.binary_cross_entropy(
        mask_scores_sigmoid, mask_labels, weight=mask_label_weight
    )
...
def discriminative_loss(
    ...
    for s in batch_size:  # range(batch_size):
        ...
        sample_loss, sample_loss_var, sample_loss_dist, sample_loss_reg = (
...
def discriminative_loss_single(
    ...
    mu_band_rep = torch.reshape(
        mu_band_rep, (num_instances[0] * num_instances[0], feature_dim)
    )
