...
class MyPanopticTracker(SegmentationTracker):
    ...
    def track(
        ...
        **kwargs,
        ...
        if self._test_area is None:
            ...
            self._test_area.ins_pre_embed = -1 * torch.ones(
                self._test_area.y.shape[0], dtype=torch.int
            )
            self._test_area.ins_pre_offset = -1 * torch.ones(
                self._test_area.y.shape[0], dtype=torch.int
            )
        ...
        self._test_area.ins_pre_embed, self._test_area.max_instance_embed = (
            self.block_merging(
        ...
        print(
            "total time for block merging of embeds:%sms"
            % ((time_for_blockMerging_embed) * 1000)
        )
        log.info(
            "total time for block merging of embeds:{}ms".format(
                (time_for_blockMerging_embed) * 1000
            )
        )
        ...
        T1 = time.perf_counter()
        self._test_area.ins_pre_offset, self._test_area.max_instance_offset = (
            self.block_merging(
                original_input_ids.cpu().numpy(),
                originids.cpu().numpy(),
        ...
        print(
            "total time for block merging of offsets:%sms"
            % ((time_for_blockMerging_offset) * 1000)
        )
        log.info(
            "total time for block merging of offsets:{}ms".format(
                (time_for_blockMerging_offset) * 1000
            )
        )
    ...
    def finalise(
        ...
        **kwargs,
        ...
        self._iou_per_class = {
            self._dataset.INV_OBJECT_LABEL[k]: v for k, v in enumerate(per_class_iou)
        }
        ...
        if ply_output:
            ...
            idx_in_cur = [
                idx
                for idx, l in enumerate(torch.argmax(full_pred, 1).numpy())
                if l in self._dataset.stuff_classes
            ]
    ...
    def get_metrics(self, verbose=False) -> Dict[str, Any]:
        ...
        metrics["{}_Iacc_embed".format(self._stage)] = meter_value(
            self._acc_meter_embed
        )
        ...
        metrics["{}_Iacc_offset".format(self._stage)] = meter_value(
            self._acc_meter_offset
        )
        ...
        if verbose and self._has_instance_data:
            metrics["{}_class_rec_embed".format(self._stage)] = self._dict_to_str(
                self._rec
            )
            metrics["{}_class_ap_embed".format(self._stage)] = self._dict_to_str(
                self._ap
            )
            metrics["{}_class_rec_offset".format(self._stage)] = self._dict_to_str(
                self._rec
            )
            metrics["{}_class_ap_offset".format(self._stage)] = self._dict_to_str(
                self._ap
            )
        return metrics
