    ...
    def track(
        ...
        **kwargs,
        ...
        if self._test_area is None:
            ...
            self._test_area.prediction_count = torch.zeros(
                self._test_area.y.shape[0], dtype=torch.int
            )
            self._test_area.votes = torch.zeros(
                (self._test_area.y.shape[0], self._num_classes), dtype=torch.float
            )
            self._test_area.ins_pre = -1 * torch.ones(
                self._test_area.y.shape[0], dtype=torch.int
            )
        ...
        if outputs.cluster_scores == None:
            ...
            # block merging
            self._test_area.ins_pre, self._test_area.max_instance = self.block_merging(
                original_input_ids.cpu().numpy(),
                originids.cpu().numpy(),
                curins_pre,
                self._test_area.ins_pre.cpu().numpy(),
                self._test_area.max_instance,
        ...
        for i, cluster in enumerate(outputs.clusters):
            ...
            write_ply(
                ...
                [
                    ...
                    self._test_area.y[originids[cluster]]
                    .detach()
                    .cpu()
                    .numpy()
                    .astype("int32"),
            ...
            if valid_c_idx != None:
                if i in valid_c_idx:
                    ...
                    write_ply(
                        val_name,
                        [
                            self._test_area.pos[originids[cluster]]
                            .detach()
                            .cpu()
                            .numpy(),
                            semantic_prob.detach().cpu().numpy(),
            ...
            if hasattr(outputs, "embed_logits"):
                ...
                write_ply(
                    ...
                    [
                        ...
                        self._test_area.instance_labels[origin_sub_ids]
                        .detach()
                        .cpu()
                        .numpy()
                        .astype("int32"),
                        ...
                        self._test_area.y[origin_sub_ids]
                        .detach()
                        .cpu()
                        .numpy()
                        .astype("int32"),
    ...
    def finalise(
        ...
        **kwargs,
        ...
        if ply_output:
            ...
            mat_pos = torch.sub(
                self._test_area.pos[has_prediction][x_idx], self._test_area.pos[y_idx]
            )
