...
def hdbscan_cluster(prediction):
    clusterer = hdbscan.HDBSCAN(
        ...
        min_samples=5,
        core_dist_n_jobs=1,
        cluster_selection_epsilon=0.006,
...
def cluster_loop(
    embed_logits_logits_u, unique_in_batch, label_batch, local_ind, low, high, loop_num
):
    # t = time.time()
    all_clusters = []
    cluster_type = []
    ...
    for loop_i in range(loop_num):
        ...
        feature_choose = torch.multinomial(
            torch.ones(embed_logits_logits_u.shape[-1]),
            pick_num[loop_i],
            replacement=False,
            out=None,
        )
...
def cluster_single(
    embed_logits_logits_u, unique_in_batch, label_batch, local_ind, type
):
    # t = time.time()
    all_clusters = []
    cluster_type = []
