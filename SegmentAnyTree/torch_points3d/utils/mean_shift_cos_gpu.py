...
def mean_shift_cosine(X, bandwidth=None, seeds=None, cluster_all=True, GPU=True):
        ...
        while True:
            ...
            for i in range(len(number)):
                if number[i] is not None:
                    center_intensity_dict[tuple(labels[i])] = number[
                        i
                    ]  # find out cluster
            ...
            nbrs = NearestNeighbors(radius=bandwidth, metric="cosine").fit(
                sorted_centers
            )
...
class MeanShiftCosine(BaseEstimator, ClusterMixin):
    ...
    def fit(self, X, y=None):
        ...
        """
        X = check_array(X)
        self.cluster_centers_, self.labels_ = mean_shift_cosine(
            X,
