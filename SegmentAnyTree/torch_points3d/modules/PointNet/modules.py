...
from torch_points3d.core.common_modules.spatial_transform import (
...
class PointNetSTN3D(BaseLinearTransformSTNkD):
    def __init__(
        self, local_nn=[3, 64, 128, 1024], global_nn=[1024, 512, 256], batch_size=1
    ):
        super().__init__(
            MiniPointNet(local_nn, global_nn), global_nn[-1], 3, batch_size
        )
...
class PointNetSTNkD(BaseLinearTransformSTNkD, BaseInternalLossModule):
    def __init__(
        ...
        super().__init__(
            MiniPointNet(local_nn, global_nn), global_nn[-1], k, batch_size
        )
    ...
    def get_internal_losses(self):
        return {
            "orthogonal_regularization_loss": self.get_orthogonal_regularization_loss()
        }
...
class PointNetSeg(torch.nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.input_stn = PointNetSTN3D(
            [input_nc] + input_stn_local_nn, input_stn_global_nn, batch_size
        )
        self.local_nn_1 = MLP([input_nc] + local_nn_1)
        self.feat_stn = PointNetSTNkD(
            feat_stn_k, feat_stn_local_nn, feat_stn_global_nn, batch_size
        )
    ...
    def forward(self, x, batch):
        ...
        if x_feat_trans.dim() == 2:
            ...
            feat_concat = torch.cat(
                [
                    x_feat_trans,
                    global_feature.unsqueeze(1).repeat((1, x_feat_trans.shape[1], 1)),
                ],
                dim=-1,
            )
