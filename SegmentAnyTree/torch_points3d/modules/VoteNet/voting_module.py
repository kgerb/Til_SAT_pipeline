...
class VotingModule(nn.Module):
    def __init__(self, vote_factor, seed_feature_dim, conv_type="DENSE"):
        ...
        if conv_type == "DENSE":
            ...
            self.conv3 = torch.nn.Linear(
                self.in_dim, (3 + self.out_dim) * self.vote_factor
            )
            self.bn1 = torch.nn.BatchNorm1d(self.in_dim)
            self.bn2 = torch.nn.BatchNorm1d(self.in_dim)
    ...
    def forward(self, data):
        ...
        if 3 < data.pos.dim() and data.pos.dim() <= 1:
            raise Exception(
                "data.pos doesn t have the correct dimension. Should be either 2 or 3"
            )
        ...
        if self._conv_type == "DENSE":
            ...
            x = x.transpose(2, 1).view(
                batch_size, num_points, self.vote_factor, 3 + self.out_dim
            )
            offset = x[:, :, :, 0:3]
            ...
            vote_x = (
                data.x.transpose(2, 1).unsqueeze(2).contiguous() + res_x.contiguous()
            )
