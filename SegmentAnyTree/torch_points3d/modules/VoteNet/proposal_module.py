...
class ProposalModule(nn.Module):
    def __init__(
        ...
        self.mean_size_arr = nn.Parameter(
            torch.Tensor(mean_size_arr), requires_grad=False
        )
        ...
        assert vote_aggregation_config.module_name == "PointNetMSGDown", (
        ...
        output_feat = (
            2 + 3 + num_heading_bin * 2 + self.num_size_cluster * 4 + self.num_class
        )
    ...
    def forward(self, data):
        ...
        if self.sampling == "seed_fps":
            ...
            raise ValueError(
                "Unknown sampling strategy: %s. Exiting!" % (self.sampling)
            )
