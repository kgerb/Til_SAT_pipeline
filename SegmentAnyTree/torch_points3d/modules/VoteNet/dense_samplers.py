...
class RandomSamplerToDense(BaseSampler):
    ...
    def sample(self, data, num_batches, conv_type):
        if conv_type == "DENSE":
            assert self._num_to_sample <= data.pos.shape[1], (
            ...
            data.pos = torch.gather(
                data.pos, 1, idx.unsqueeze(-1).repeat(1, 1, data.pos.shape[-1])
            )
            data.x = torch.gather(
                data.x, 2, idx.unsqueeze(1).repeat(1, data.x.shape[1], 1)
            )
            return data, idx
        else:
            pos = []
            ...
            for batch_idx in range(num_batches):
                ...
                assert self._num_to_sample <= pos_masked.shape[0], (
                    "self._num_to_sample: {} should be smaller than num_pos: {}".format(
        ...
        if conv_type == "DENSE":
            ...
            for i in range(num_batches):
                idx = fps(
                    data.pos[i],
                    batch,
                    ratio=self._get_ratio_to_sample(data.pos.shape[1]),
                )
                idx_out.append(idx)
            idx_out = torch.stack(idx_out)
            pos = torch.gather(
                data.pos, 1, idx_out.unsqueeze(-1).repeat(1, 1, data.pos.shape[-1])
            )
            x = torch.gather(
                data.x, 2, idx_out.unsqueeze(1).repeat(1, data.x.shape[1], 1)
            )
            ...
            for batch_idx in range(num_batches):
                ...
                if self._num_to_sample >= pos_masked.shape[0]:
                    ...
                    idx = fps(
                        pos_masked,
                        batch,
                        ratio=self._get_ratio_to_sample(pos_masked.shape[0]),
                    )
                    pos.append(pos_masked[idx])
                    x.append(x_masked[idx])
                    idx_out.append(idx + num_points)
