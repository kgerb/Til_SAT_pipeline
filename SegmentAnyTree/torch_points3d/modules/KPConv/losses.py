...
def repulsion_loss(deformed_kpoints, radius):
    ...
    for i in range(n_points):
        with torch.no_grad():
            other_points = torch.cat(
                [deformed_kpoints[:, :i, :], deformed_kpoints[:, i + 1 :, :]], dim=1
            )
        distances = torch.sqrt(
            torch.sum((other_points - deformed_kpoints[:, i : i + 1, :]) ** 2, dim=-1)
        )
...
def permissive_loss(deformed_kpoints, radius):
    ...
    permissive_loss = torch.mean(
        norm_deformed_normalized[norm_deformed_normalized > 1.0]
    )
