...
class Viz:
    ...
    def _get_cvs_df(self) -> dict:
        ...
        print(
            "Type of DataFrame column name:", type(csv_df.columns[2])
        )  # Assuming 'gt_label(dominant_label)' is the third column
    ...
    def main(self):
        ...
        for gt_label in csv_dict:
            ...
            pred_df_filtered = pred_df[
                pred_df["preds_instance_segmentation"] == pred_label
            ]
            ...
            outliers_df["B"] = 0
            outliers_df.to_csv(
                f"{self.output_path}/{gt_label}_{pred_label}_outliers.csv"
            )

            if self.verbose:
                num_outliers = len(outliers_df)
                print(
                    f"gt_label: {gt_label}, pred_label: {pred_label}, num_outliers: {num_outliers}"
                )
