...
class VizualizeSmallTrees(object):
    ...
    def compute_mean_metrics(self, gt_trees, df_csv):
        ...
        if df_csv.shape[0] == 0:
            ...
            commission_rate = (
                df_csv.loc[df_csv["IoU"] <= 0.5].shape[0] / df_csv.shape[0]
            )
        ...
        dict_metrics["predicted_trees"] = [
            x - 1 for x in df_csv["pred_label"].tolist()
        ]  # subtract 1 from pred_label to get the index of the tree
        dict_metrics["predicted_trees_ok"] = [
            x - 1 for x in df_csv.loc[df_csv["IoU"] > 0.5]["pred_label"].tolist()
        ]  # subtract 1 from pred_label to get the index of the tree
    ...
    def read_single_csv_and_parse(self, laz_and_csv_file):
        ...
        gt_trees_with_height_max = (
            df_laz[["treeID", "Z"]].groupby("treeID").max().reset_index()
        )
        gt_trees_with_height_min = (
            df_laz[["treeID", "Z"]].groupby("treeID").min().reset_index()
        )
        ...
        gt_trees_with_height["Z"] = (
            gt_trees_with_height_max["Z"] - gt_trees_with_height_min["Z"]
        )

        # convert gt_trees_with_height to list of tuples
        gt_trees_with_height = list(
            zip(
                gt_trees_with_height["treeID"].tolist(),
                gt_trees_with_height["Z"].tolist(),
            )
        )
        ...
        for key, value in HIGH_RANGES_DICT.items():
            df_csv_in_range = df_csv.loc[
                (df_csv["high_of_tree_gt"] >= HIGH_RANGES[key])
                & (df_csv["high_of_tree_gt"] < HIGH_RANGES[key + 1])
            ]

            # find the gt trees in gt_trees_with_height that are in the range
            gt_trees_in_range = [
                x[0]
                for x in gt_trees_with_height
                if x[1] >= HIGH_RANGES[key] and x[1] < HIGH_RANGES[key + 1]
            ]
        ...
        for key in range_dict.keys():
            ...
            print(
                "num predicted trees in metric: ",
                range_dict[key]["num_predicted_trees"],
            )
            print("")
        ...
        for key in range_dict.keys():
            ...
            if (
                len(range_dict[key]["gt_trees"]) > 0
                and len(range_dict[key]["predicted_trees"]) > 0
                and len(range_dict[key]["predicted_trees_ok"]) > 0
            ):
                print("len gt trees: ", len(range_dict[key]["gt_trees"]))
                print("len predicted trees: ", len(range_dict[key]["predicted_trees"]))
                print(
                    "len predicted trees ok: ",
                    len(range_dict[key]["predicted_trees_ok"]),
                )
    ...
    def generate_2D_plot_of_trees(
        ...
        # Set the grid
        plt.xticks(
            np.arange(axis_limits[0], axis_limits[1], step=grid_interval, dtype=int)
        )
        plt.yticks(
            np.arange(axis_limits[2], axis_limits[3], step=grid_interval, dtype=int)
        )
        # plt.grid(True, linestyle='-', alpha=0.7)
        ...
        for gt_tree in list_of_gt_trees:
            ...
            if len(x) > 0 and len(y) > 0:  # Check if arrays are not empty
                ...
                plt.fill(
                    ...
                    edgecolor="none",
                    linewidth=2,
                    label="Ground Truth Trees"
                    if gt_tree == list_of_gt_trees[0]
                    else "",
                )

        for pred_tree_ok in list_of_pred_trees_ok:
            df_laz_pred_tree_ok = df_laz.loc[
                df_laz["preds_instance_segmentation"] == pred_tree_ok
            ]
            x, y = (
                df_laz_pred_tree_ok["X"].to_numpy(),
                df_laz_pred_tree_ok["Y"].to_numpy(),
            )
            if len(x) > 0 and len(y) > 0:
                ...
                plt.fill(
                    ...
                    edgecolor="none",
                    linewidth=2,
                    label="Predicted Trees OK"
                    if pred_tree_ok == list_of_pred_trees_ok[0]
                    else "",
                )

        for pred_tree in [
            item for item in list_of_pred_trees if item not in list_of_pred_trees_ok
        ]:
            df_laz_pred_tree = df_laz.loc[
                df_laz["preds_instance_segmentation"] == pred_tree
            ]
            ...
            if len(x) > 0 and len(y) > 0:  # Check if arrays are not empty
                ...
                plt.fill(
                    ...
                    if pred_tree == list_of_pred_trees[0]
                    else "",
                )
    ...
    def read_multiple_files(self, folder_name):
        ...
        for subfolder in subfolders:
            ...
            csv_files = [
                f.path
                for f in os.scandir(os.path.join(subfolder, "metrics_out"))
                if f.is_file()
                and f.name.endswith(".csv")
                and f.name != "summary_metrics_all_plots.csv"
            ]
            ...
            laz_files = [
                ...
                if f.is_file() and f.name.endswith(".laz")
            ]
            ...
            for laz_file in laz_files:
                ...
                matching_csv = next(
                    (
                        ...
                        if os.path.splitext(os.path.basename(csv))[0] == base_name
                    ),
                    None,
                )
                if matching_csv:
            ...
            Parallel(n_jobs=-1)(
                delayed(self.read_single_csv_and_parse)(file)
                for file in laz_and_csv_files
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-i", "--folder", type=str, required=True, help="Path to folder with csv files"
    )
