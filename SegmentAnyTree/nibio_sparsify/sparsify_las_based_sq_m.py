...
class SparsifyLasBasedSqM:
    def __init__(
        self, input_file, output_folder=None, target_density=10, verbose=False
    ):
        ...
        self.logger.info(
            f"Initialized with input file: {self.input_file}, target density: {self.target_density}"
        )
    ...
    def sparsify(self, point_cloud):
        ...
        self.logger.info(
            f"Point cloud density: {self.density} points per square meter."
        )

        # Check if the point cloud density is already less than or equal to the target density
        if self.density <= self.target_density:
            self.logger.info(
                "Point cloud is already sparser than the target density. Skipping sparsification."
            )
            return (
                point_cloud.points
            )  # Return the original points without sparsification
        ...
        self.new_point_cloud_density = self.calculate_density_convex_hull(
            filtered_point_cloud
        )
        self.logger.info(
            f"Reduced point cloud size from {len(x)} to {len(filtered_point_cloud)} points."
        )
        self.logger.info(
            f"Reduced point cloud by {(1 - len(filtered_point_cloud) / len(x)) * 100}%."
        )
        self.logger.info(
            f"New point cloud density: {self.new_point_cloud_density} points per square meter."
        )
    ...
    def process(self):
        ...
        ):  # If output_file is not set, use the default naming
            self.output_file = os.path.join(
                self.output_folder,
                os.path.basename(self.input_file).replace(
...
class SparsifyLasBasedSqMInFolder(SparsifyLasBasedSqM):
    def __init__(
        self, input_folder, output_folder=None, target_density=10, verbose=False
    ):
        super().__init__(
            input_file=None,
    ...
    def reduce_point_clouds(self):
        ...
        for point_cloud_path in tqdm(point_cloud_paths):
            relative_path = os.path.relpath(
                point_cloud_path, self.directory_with_point_clouds
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-i", "--input_folder", help="The folder with .las files to sparsify."
    )
    parser.add_argument(
        "-o",
        "--output_folder",
        default=None,
        help="The folder where the sparse point clouds will be saved.",
    )
    parser.add_argument(
        "-d",
        "--target_density",
        help="The target density in points per square meter.",
        default=10,
        type=int,
    )
    parser.add_argument(
    ...
    sparsifier = SparsifyLasBasedSqMInFolder(
        args.input_folder, args.output_folder, args.target_density, args.verbose
    )
