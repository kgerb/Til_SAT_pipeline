...
def split_pointcloud(df, x_step, y_step, overlap=0.0, verbose=False):
    ...
    while x_current < x_max:
        ...
        while y_current < y_max:
            ...
            chunk = df[
                (df["X"] >= x_current)
                & (df["X"] < x_current + x_step)
                & (df["Y"] >= y_current)
                & (df["Y"] < y_current + y_step)
            ]
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Split point clouds in chunks based on x, y steps and overlap."
    )
    parser.add_argument(
        "-i",
        "--input_dir",
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Output directory to save chunked point cloud files.",
    )
    parser.add_argument(
        "--x_step",
        type=float,
        required=False,
        default=10.0,
        help="Width of the grid cell along the X-axis.",
    )
    parser.add_argument(
        "--y_step",
        type=float,
        required=False,
        default=10.0,
        help="Height of the grid cell along the Y-axis.",
    )
    parser.add_argument(
        "--overlap",
        type=float,
        default=0.0,
        help="Overlap fraction (e.g., 0.2 means 20% overlap). Default is 0.0. Must be between 0 and 1.",
    )
    ...
    for file_name in tqdm(os.listdir(input_dir)):
        ...
        for i, chunk in enumerate(chunks):
            output_file = os.path.join(
                output_dir, f"{os.path.splitext(file_name)[0]}_chunk_{i}.ply"
            )
