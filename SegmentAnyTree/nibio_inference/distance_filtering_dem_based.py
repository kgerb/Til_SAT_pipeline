...
class DistanceFilteringDemBased(object):
    ...
    def compute_distance_between_dem_and_target(self, dem, target_points):
        ...
        original_target_points["z_distance"] = original_target_points[
            "z_distance"
        ].values
    ...
    def filter_points(self, target_points_with_distances):
        # filter points
        target_points_with_distances = target_points_with_distances[
            target_points_with_distances["z_distance"] < self.distance
        ]
    ...
    def run(self):
        ...
        points_df, ground_points, target_points = self.read_las_and_put_to_pandas(
            input_file=self.point_cloud_file
        )

        dem = self.compute_dem_for_ground(ground_points=ground_points)

        target_points_with_distances = self.compute_distance_between_dem_and_target(
            dem=dem.values, target_points=target_points
        )

        filtered_points = self.filter_points(
            target_points_with_distances=target_points_with_distances
        )

        points_df = self.update_las_file(
            points_df=points_df, target_points_with_distances=filtered_points
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-d",
        "--distance",
        help="Distance in meters e.g. 0.5",
        default=0.5,
        required=False,
        type=float,
    )
