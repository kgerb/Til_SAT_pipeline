...
class MergePtSsIsInFolders(object):
    ...
    def merge_file(self, item):
        MergePtSsIs(
            ...
            output_file_path=os.path.join(
                ...
                os.path.basename(item[0]).split(".")[0] + ".laz",
            ),
    ...
    def merge(self):
        ...
        segmented_data_files = glob.glob(
            os.path.join(self.segmented_data_folder_path, "*.ply")
        )

        # combine the input_data_folder_path and the core names of the files in the input_data_folder_path
        input_data = {
            os.path.basename(file).split(".")[0]: file for file in input_data_files
        }

        # find all the _semantic_segmentation.ply files in the segmented_data_folder_path
        semantic_segmentation_files = [
            file
            for file in segmented_data_files
            if "_semantic_segmentation.ply" in file
        ]
        ...
        semantic_segmentation = {
            ...
            .split(".")[0]
            .replace("_semantic_segmentation", ""): file
            for file in semantic_segmentation_files
        }

        # find all the _instance_segmentation.ply files in the segmented_data_folder_path
        instance_segmentation_files = [
            file
            for file in segmented_data_files
            if "_instance_segmentation.ply" in file
        ]
        ...
        instance_segmentation = {
            os.path.basename(file)
            .split(".")[0]
            .replace("_instance_segmentation", ""): file
            for file in instance_segmentation_files
        }

        list_of_matched_tupeles = []
        for item in input_data.keys():
            if (
                item in semantic_segmentation.keys()
                and item in instance_segmentation.keys()
            ):
                list_of_matched_tupeles.append(
                    (
                        input_data[item],
                        semantic_segmentation[item],
                        instance_segmentation[item],
                    )
                )

        # Use joblib's Parallel and delayed to run in parallel
        Parallel(n_jobs=-1)(
            delayed(self.merge_file)(item) for item in tqdm(list_of_matched_tupeles)
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Merge point cloud, semantic segmentation and instance segmentation."
    )
    parser.add_argument(
        "-i",
        "--input_data_folder_path",
    ...
    parser.add_argument(
        ...
        "--segmented_data_folder_path",
        type=str,
        help="Path to the segmented data folder.",
    )
    parser.add_argument(
        "-o",
        "--output_data_folder_path",
