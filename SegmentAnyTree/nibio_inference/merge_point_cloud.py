...
def process_chunk(chunk, prediction="preds"):
    ...
    chunk["x_border"] = (
        (chunk["x"] <= x_min + border_limit) | (chunk["x"] >= x_max - border_limit)
    ).astype(int)
    chunk["y_border"] = (
        (chunk["y"] <= y_min + border_limit) | (chunk["y"] >= y_max - border_limit)
    ).astype(int)
    ...
    chunk["border_tree"] = (
        chunk[prediction].isin(unique_values_border_trees).astype(int)
    )
    ...
    for value in unique_values_border_trees:
        ...
        chunk.loc[
            chunk[prediction] == value, ["x_centroid", "y_centroid", "z_centroid"]
        ] = centroid_value
...
def merge_pointclouds(chunk_name, chunks_list, prediction="preds"):
    results = Parallel(n_jobs=-1, prefer="threads")(
        ...
        for chunk in tqdm(chunks_list, desc="Computing centroids")
    )
    ...
    for index, row in results_df.iterrows():
        ...
        mapping = {
            ...
            for old_val, new_val in zip(
                row["unique_values"],
                range(start_value, start_value + row["number_of_unique_values"]),
            )
        }
    ...
    concatenated["x_border_global"] = (
        (concatenated["x"] <= concatenated["x"].min() + 0.05)
        | (concatenated["x"] >= concatenated["x"].max() - 0.05)
    ).astype(int)
    concatenated["y_border_global"] = (
        (concatenated["y"] <= concatenated["y"].min() + 0.05)
        | (concatenated["y"] >= concatenated["y"].max() - 0.05)
    ).astype(int)

    # find for border trees in concatenated point cloud
    border_trees_global = concatenated[
        (concatenated["x_border_global"] == 1) | (concatenated["y_border_global"] == 1)
    ]
    ...
    concatenated["border_tree_global"] = (
        concatenated[prediction].isin(unique_values_border_trees_global).astype(int)
    )
    (
        concatenated["x_centroid_global"],
        concatenated["y_centroid_global"],
        concatenated["z_centroid_global"],
    ) = 0, 0, 0
    ...
    all_centroids = (
        concatenated[concatenated["border_tree"] == 1]
        .groupby(prediction)
        .first()[["x_centroid", "y_centroid", "z_centroid"]]
    )
    ...
    for chunk, x_item, y_item in zip(chunks_list, x_min_max, y_min_max):
        for centroid in all_centroids.values:
            if (
                x_item[0] <= centroid[0] <= x_item[1]
                and y_item[0] <= centroid[1] <= y_item[1]
            ):
                centroids_by_chunk[(x_item, y_item)].append(centroid)
                # Remove the centroid from all_centroids to ensure it's not counted again
                all_centroids = all_centroids.drop(
                    all_centroids[
                        (all_centroids["x_centroid"] == centroid[0])
                        & (all_centroids["y_centroid"] == centroid[1])
                    ].index
                )
...
def main():
    parser = argparse.ArgumentParser(
        description="Merge point cloud chunks into a single point cloud."
    )
    parser.add_argument(
        "-i",
        "--input_dir",
        type=str,
        required=True,
        help="Input directory containing chunked point cloud files.",
    )
    parser.add_argument(
        "-o",
        "--output_dir",
        type=str,
        required=True,
        help="Output directory to save merged point clouds.",
    )
