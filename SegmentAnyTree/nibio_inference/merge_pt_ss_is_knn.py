...
class MergePtSsIsKnn(object):
    ...
    def merge(self):
        ...
        df_semantic_segmentation_matched = semantic_segmentation_df.iloc[
            indices_semantic
        ]
        ...
        df_semantic_segmentation_matched.to_csv(
            self.output_file_path.replace(".laz", "_semantic_segmentation_matched.csv"),
            index=False,
        )
        ...
        df_instance_segmentation_matched = instance_segmentation_df.iloc[
            indices_instance
        ]
        ...
        df_instance_segmentation_matched.to_csv(
            self.output_file_path.replace(".laz", "_instance_segmentation_matched.csv"),
            index=False,
        )

        # Rename columns for semantic and instance segmentations
        df_semantic_segmentation_matched.columns = [
            f"{col}_semantic_segmentation" for col in semantic_segmentation_df.columns
        ]
        df_instance_segmentation_matched.columns = [
        ...
        merged_df = pd.concat(
            [merged_df, df_semantic_segmentation_matched.drop(columns=["x", "y", "z"])],
            axis=1,
        )
