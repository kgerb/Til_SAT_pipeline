...
from nibio_inference.las_to_pandas import (
    las_to_pandas,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Revert the modification of ply and las files."
    )
    parser.add_argument(
        "-i",
        "--input_folder",
        type=str,
        help="Path to the input folder containing modified files.",
    )
    parser.add_argument(
        "-m",
        "--min_values_folder",
        type=str,
        help="Path to the folder containing min_values.json files.",
    )
    parser.add_argument(
        "-o",
        "--output_folder",
        type=str,
        help="Path to the output folder to save reverted files.",
    )
    ...
    for filename in tqdm(
        os.listdir(args.input_folder), desc="Reverting files", unit="file"
    ):
        input_file_path = os.path.join(args.input_folder, filename)
        min_values_path = os.path.join(
            args.min_values_folder, os.path.splitext(filename)[0] + "_min_values.json"
        )
