...
def las_to_ply(
    ...
    X, Y, Z = (
        get_coord(las.X, scale_x),
        get_coord(las.Y, scale_y),
        get_coord(las.Z, scale_z),
    )
    ...
    print(
        "{} percent of the points are ground points.".format(
            (np.count_nonzero(las.classification == 2) / len)
        )
    )
    print(
        "{} percent of the points are low vegetation points.".format(
            (np.count_nonzero(las.classification == 1) / len)
        )
    )
    print(
        "{} percent of the points are outpoints.".format(
            (np.count_nonzero(las.classification == 3) / len)
        )
    )
    ...
    classification_new = las.classification[
        points_to_keep
    ]  # classification without points that should be removed
    ...
    path_to_region = (
        ply_file_path.parents[4]
        .joinpath(ply_file_path.parts[-5] + foldername_addition)
        .joinpath(*ply_file_path.parts[-4:-1])
    )
    ...
    sem_seg[classification_new == 3] = (
        0.0  # outpoints (trees outside of the measured/annotated plots) -> unclassified
    )
    ...
    el = PlyElement.describe(
        data_struct, "vertex", comments=["Created manually from las files."]
    )
...
if __name__ == "__main__":
    random.seed(
        42
    )  # set seed so that validation set gets chosen randomly, but fixed from within the files annotated as
    ...
    rel_path_list, forest_region_list, split_list = train_val_test_split(
        train_test_split_path
    )  # creates train-val-test split from train-test split
    # TO ADAPT: path where the code folder "OutdoorPanopticSeg_V2" is located
    code_basepath = "/local/home/vfrawa/Documents/code"
    codes_data_basepath = Path(
        code_basepath + "/OutdoorPanopticSeg_V2/data/treeinsfused/raw"
    )  # this is where the ply files should be located so that the code accesses them
    ...
    for i in range(len(rel_path_list)):  # per .las data file
        ...
        ply_file_path = codes_data_basepath.joinpath(las_file_path.parts[-2]).joinpath(
            forest_region_list[i]
            + "_"
            + las_file_path.stem
            + "_"
            + split_list[i]
        ...
        ply_file_path_datanamechange = las_to_ply(
            las_file_path, ply_file_path, remove_ground, remove_lowveg, remove_outpoints
        )
    ...
    print(
        testpath_list
    )  # list of paths of all files used as test files -> can be used for fold in conf/eval.yaml
